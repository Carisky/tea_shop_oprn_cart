			...
			*/
			...
			foreach ($results as $result) {
				...
				if ($result['image1']) { // Noir
					$image1 = $this->model_tool_image->resize($result['image1'], $this->config->get('configblog_image_related_width'), $this->config->get('configblog_image_related_height'));
				} else {
					$image1 = '';
				}
			...
			// < Noir
			$data['heading_title'] = $article_info['name'];
			$data['subtitle'] = $article_info['meta_h1'];
			$data['cat_href'] = isset($this->request->get['blog_category_id']) ? $this->url->link('blog/category', 'blog_category_id='.$this->request->get['blog_category_id']) : '';
			
			if ($article_info['image']) {
				$data['thumb'] = $this->model_tool_image->resize($article_info['image'], $this->config->get('configblog_image_category_width'), $this->config->get('configblog_image_category_height'));
				$data['thumb_mob'] = $this->model_tool_image->resize($article_info['image'], 767, 320, 1);
			} else {
				$data['thumb'] = '';
			}
			...
			if(!empty($blog_category_id)) {
				$filter_blog_category_id = explode('_', $blog_category_id);
				$article_data = array(
					'blog_category_id' => (int)end($filter_blog_category_id),
					'article_id' => $article_id,
					'limit'              => 3
				);
				$results = $this->model_blog_article->getArticleArticles($article_data);
