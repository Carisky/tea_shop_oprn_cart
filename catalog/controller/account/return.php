...
class ControllerAccountReturn extends Controller {
	...
	public function index() {
		...
		foreach ($results as $result) {
			...
			*/
			...
			$data['returns'][] = array_merge($result, [
				'href'       => $this->url->link('account/return/info', 'return_id=' . $result['return_id'] . $url, true),
	...
	// Noir
	public function nr_add()
	{
		if (!$this->customer->isLogged()) $this->response->redirect($this->url->link('common/home', '', true));

		$this->load->language('account/return');

		$this->load->model('account/return');
		
		if(!$this->request->get['product'] or !$this->request->get['order_id']) $this->response->redirect($this->url->link('common/home', '', true));
		...
		$data['breadcrumbs'][] = array(
			'text' => $this->language->get('heading_title'),
			'href' => $this->url->link('account/return/nr_add', '', true)
		);

		$data['order_info'] = $this->model_account_order->getOrder($this->request->get['order_id']);
		$data['product_info'] = $this->model_account_order->getOrderProduct($this->request->get['order_id'], $this->request->get['product']);
		$data['return_reasons'] = $this->model_localisation_return_reason->getReturnReasons();
		$data['date_ordered'] = date('Y-m-d', strtotime($data['order_info']['date_added']));
	...
	public function nr_save()
	{
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') and $this->validate()) {
			if(!$this->request->post['quantity']) $this->request->post['quantity'] = 1;
			$this->load->model('account/return');
			$this->model_account_return->addReturn($this->request->post);
			$json = ['redirect' => $this->url->link('account/return', '', true)];
		} else {
