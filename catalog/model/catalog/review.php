...
class ModelCatalogReview extends Model {
	public function addReview($product_id, $data) {
		...
		if (in_array('review', (array)$this->config->get('config_mail_alert'))) {
			...
			$message .= sprintf($this->language->get('text_rating'), $data['rating'].'-'.$data['rating1'].'-'.$data['rating2']) . "\n"; // Noir
	...
	public function getNrReviews($product_id, $page = 1, $limit = 8)  // Noir
	{
		if ($limit < 1) $limit = 8;
		$start = ($page < 2) ? 0 : ($page - 1)*$limit;
		
		$query = $this->db->query("
			SELECT * FROM " . DB_PREFIX . "review 
