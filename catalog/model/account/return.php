...
class ModelAccountReturn extends Model {
	...
	public function getReturns($start = 0, $limit = 20) {
		...
		$query = $this->db->query("
			SELECT r.*, rs.name as status, p.image  
			FROM `".DB_PREFIX."return` AS r 
			LEFT JOIN ".DB_PREFIX."return_status AS rs ON (r.return_status_id = rs.return_status_id) 
			LEFT JOIN ".DB_PREFIX."product AS p ON (r.product_id = p.product_id) 
			WHERE r.customer_id = ".(int)$this->customer->getId()." AND rs.language_id = ".(int)$this->config->get('config_language_id')." 
			ORDER BY r.return_id DESC 
			LIMIT ".(int)$start.",".(int)$limit); //Noir
		return $query->rows;
	}
	...
	public function nrCheckReturn($order_id, $product_id) // Noir
	{
		$query = $this->db->query("SELECT 1 FROM `" . DB_PREFIX . "return` WHERE order_id = ".(int)$order_id." AND product_id = ".(int)$product_id);
