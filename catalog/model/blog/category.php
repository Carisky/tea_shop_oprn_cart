...
class ModelBlogCategory extends Model {
	...
	{
		$query = $this->db->query("
			SELECT * FROM " . DB_PREFIX . "blog_category AS c 
			LEFT JOIN " . DB_PREFIX . "blog_category_description AS cd ON (c.blog_category_id = cd.blog_category_id) 
			LEFT JOIN " . DB_PREFIX . "blog_category_to_store AS c2s ON (c.blog_category_id = c2s.blog_category_id) 
