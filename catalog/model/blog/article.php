...
class ModelBlogArticle extends Model {
	...
	public function getTotalArticles($data = array()) {
		...
		if (!$article_data) {
			...
			if (!empty($data['filter_blog_category_id'])) {
				//if (!empty($data['filter_sub_blog_category'])) { //Noir
				if (!empty($data['filter_sub_category'])) {	//Noir
					$implode_data = array();
					...
					//$categories = $this->model_blog_category->getCategoriesByParentId($data['filter_category_id']); //Noir
					$categories = $this->model_blog_category->getCategoriesByParentId($data['filter_blog_category_id']); //Noir
	...
	//Noir
	public function getArticleArticles($data) 
	{
		$cache = 'article.articles.'.(int)$this->config->get('config_language_id').'.'.(int)$this->config->get('config_store_id').'.'.$data['article_id'];
		$article_data = $this->cache->get($cache);
		if ($article_data) return $article_data;
			
		$query = $this->db->query("
			SELECT p.article_id, p.image, d.name 
			FROM ".DB_PREFIX."article AS p 
			LEFT JOIN ".DB_PREFIX."article_to_store AS p2s ON (p.article_id = p2s.article_id) 
			LEFT JOIN ".DB_PREFIX."article_description AS d ON (p.article_id = d.article_id) 
			WHERE p.status = '1' AND p2s.store_id = ".(int)$this->config->get('config_store_id')." AND d.language_id =  ".(int)$this->config->get('config_language_id')." 
				AND p.article_id <> ".(int)$data['article_id']." 
