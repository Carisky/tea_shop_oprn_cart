...
function Shop_OrdersGet($request)
{
	...
	//AG $sql = "SELECT `value` FROM `${dbp}setting` WHERE `key` = 'pp_standard_completed_status_id' OR `key` = 'payment_pp_standard_completed_status_id'";
	...
	while($order = DB_Fetch($res))
	{
		...
		elseif (preg_match('/payu|dotpay|tpay|pp_standard|paypal|cashbill|eway|przelewy24|comgate|bluepayment|stripe|gpwebpaygpebinder'.$mapped_payments.'/i', $order['payment_code']))
		{
