...
class ControllerInformationContact extends Controller {
	...
	public function index() {
		...
		$data['comment'] = $this->config->get('config_comment');
				...
				$data['me_fb_events_track_contact'] = $this->config->get('module_me_fb_events_track_contact');
...
$this->document->setSchema($schema);
		...
		$data['instagram'] = $this->config->get('config_instagram'); //Noir
		$data['store_email'] = $this->config->get('config_email'); //Noir
...
$this->document->setSchema($schema);
	...
	{
		$this->load->language('information/contact');
		if ((utf8_strlen($this->request->post['firstname']) < 3) or (utf8_strlen($this->request->post['firstname']) > 32)) {
			$this->error['name'] = $this->language->get('error_name');
		}
		if ((utf8_strlen($this->request->post['enquiry']) < 10) or (utf8_strlen($this->request->post['enquiry']) > 3000)) {
			$this->error['enquiry'] = $this->language->get('error_enquiry');
		}
		if (!filter_var($this->request->post['email'], FILTER_VALIDATE_EMAIL)) {
			$this->error['email'] = $this->language->get('error_email');
		}
		if(empty($this->request->post['agree'])) $this->error['agree'] = $this->language->get('error_agree');
		if($this->error) {
		...
		$name = $this->request->post['tname'];
		$subject = $name.' ('.$this->request->post['email'].($this->request->post['phone'] ? ', '.$this->request->post['phone'] : '').')';
		...
		$mail->setSubject(html_entity_decode(sprintf($this->language->get('email_subject'), $subject), ENT_QUOTES, 'UTF-8'));
		$mail->setText($this->request->post['enquiry']);
		$mail->send();
		
		header("Content-type: application/json");
		echo json_encode(['success' => $this->language->get('text_success')]);
		exit();
	}
}
