...
class ControllerExtensionModuleMefbevents extends Controller {
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			if(isset($this->request->post['save_stay']) && $this->request->post['save_stay'] == 1){
				$this->response->redirect($this->url->link('extension/module/me_fb_events', 'user_token=' . $this->session->data['user_token'] . '&type=module', true));
			}
		...
		$data['action'] = $this->url->link('extension/module/me_fb_events', 'user_token=' . $this->session->data['user_token'] . '&store_id='.$store_id, true);
		...
		if (isset($this->request->post['module_me_fb_events_access_token'])) {
			$data['module_me_fb_events_access_token'] = $this->request->post['module_me_fb_events_access_token'];
		} elseif(!empty($store_info['module_me_fb_events_access_token'])) {
			$data['module_me_fb_events_access_token'] = $store_info['module_me_fb_events_access_token'];
		}else{
			$data['module_me_fb_events_access_token'] = $this->config->get('module_me_fb_events_access_token');
		}

		if (isset($this->request->post['module_me_fb_events_test_event'])) {
			$data['module_me_fb_events_test_event'] = $this->request->post['module_me_fb_events_test_event'];
		} elseif(!empty($store_info['module_me_fb_events_test_event'])) {
			$data['module_me_fb_events_test_event'] = $store_info['module_me_fb_events_test_event'];
		}else{
			$data['module_me_fb_events_test_event'] = $this->config->get('module_me_fb_events_test_event');
		}
		...
		foreach($module_me_fb_events_feeds as $key => $module_me_fb_events_feed){
			...
			$data['module_me_fb_events_feeds'][$key] = array(
				...
				'minproductid' => isset($module_me_fb_events_feed['minproductid']) ? $module_me_fb_events_feed['minproductid'] : '',
				'maxproductid' => isset($module_me_fb_events_feed['maxproductid']) ? $module_me_fb_events_feed['maxproductid'] : '',
				'fromlimit' => isset($module_me_fb_events_feed['fromlimit']) ? $module_me_fb_events_feed['fromlimit'] : '',
				'tolimit' => isset($module_me_fb_events_feed['tolimit']) ? $module_me_fb_events_feed['tolimit'] : '',
				'sortby' => isset($module_me_fb_events_feed['sortby']) ? $module_me_fb_events_feed['sortby'] : '',
				'orderby' => isset($module_me_fb_events_feed['orderby']) ? $module_me_fb_events_feed['orderby'] : '',
				'sale_price_status' => isset($module_me_fb_events_feed['sale_price_status']) ? $module_me_fb_events_feed['sale_price_status'] : '',
				'sale_from' => isset($module_me_fb_events_feed['sale_from']) ? $module_me_fb_events_feed['sale_from'] : '',
				'sale_to' => isset($module_me_fb_events_feed['sale_to']) ? $module_me_fb_events_feed['sale_to'] : '',
	...
	public function getCategories(){
		...
		$domain = 'https://demo.martextensions.com/';
		...
		if (isset($this->request->get['filter_name'])) {
			...
			if($this->request->get['filter_name']){
				...
				if ($key != 0) {
					if (isset($Word)) {
					  $category_info = explode('-',$Word);
					   $category_id = isset($category_info[0]) ? trim($category_info[0]) : $key;
