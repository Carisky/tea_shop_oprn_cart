...
class ControllerCatalogAttribute extends Controller {
	...
	protected function getForm() {
		...
		if (isset($this->request->post['values_list'])) { //Noir
			$data['values_list'] = $this->request->post['values_list'];
		} elseif (!empty($attribute_info)) {
			$data['values_list'] = $attribute_info['values_list'];
		} else {
			$data['values_list'] = '';
		}
		
		if (isset($this->request->post['hidden'])) { //Noir
			$data['hidden'] = $this->request->post['hidden'];
		} elseif (!empty($attribute_info)) {
			$data['hidden'] = $attribute_info['hidden'];
		} else {
			$data['hidden'] = 0;
		}
