...
class ControllerCatalogProduct extends Controller {
	...
	protected function getForm() {
		...
		if (isset($data['tax_rule'][0]['tax_rate_id'])) {
		...
		foreach ($product_attributes as $product_attribute) {
			...
			if ($attribute_info) {
				// < Noir
				$vlist = array();
				$descriptions = $this->model_catalog_attribute->getAttributeDescriptions($product_attribute['attribute_id']);
				foreach ($descriptions as $key=> $description) {
					$avalues = $description['values_list'] ? explode(PHP_EOL, $description['values_list']) : '';
					if($avalues) {
		...
		foreach ($product_images as $product_image) {
			...
			$data['product_images'][] = array(
				...
				'in_desc'	 => $product_image['in_desc'], //Noir
	...
	public function attr_values() //Noir
	{
		$id = $this->request->get['id'];
		$row = $this->request->get['row'];
		$this->load->model('catalog/attribute');
		$descriptions = $this->model_catalog_attribute->getAttributeDescriptions($id);
		...
		foreach($languages as $language) {
			...
			<div class="input-group">
				...
				<select class="form-control" data-atribute-vals="'.$row.'" data-lang="'.$language['language_id'].'">
					<option value="">---</option>';
			foreach($vlist[$language['language_id']] as $value) {
